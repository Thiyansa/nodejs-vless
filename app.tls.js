...
const server = createServer(options, (req, res) => {
    ...
    if (parsedUrl.pathname === '/') {
        ...
        const welcomeInfo = `
            <div style="text-align: center; font-family: 'Segoe UI', Tahoma, sans-serif; padding: 30px; background: #f0f2f5; min-height: 100vh;">
                <div style="max-width: 600px; margin: auto; background: white; padding: 30px; border-radius: 20px; box-shadow: 0 15px 35px rgba(0,0,0,0.1);">
                    ...
                    <h3 style="color: #5f6368; font-weight: 400; margin-top: 0;">TLS Node Dashboard</h3>
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px; margin: 25px 0;">
                        ...
                        <div style="background: #fff5f5; padding: 15px; border-radius: 12px; border: 1px solid #feb2b2;">
                            ...
                            <span style="font-size: 1.1rem; color: #9b2c2c; font-weight: bold;">${formatBytes(stats.upload)}</span>
                        </div>
                        <div style="background: #f0fff4; padding: 15px; border-radius: 12px; border: 1px solid #9ae6b4;">
                            ...
                            <span style="font-size: 1.1rem; color: #22543d; font-weight: bold;">${formatBytes(stats.download)}</span>
                        </div>
                    </div>
...
function parseHandshake(buf) {
    ...
    offset += 1;
    let host;
    if (addressType === 1) { host = Array.from(buf.subarray(offset, offset + 4)).join('.'); offset += 4; }
    else if (addressType === 2) { const len = buf.readUInt8(offset++); host = buf.subarray(offset, offset + len).toString(); offset += len; }
    else if (addressType === 3) {
        const segments = [];
        for (let i = 0; i < 8; i++) { segments.push(buf.readUInt16BE(offset).toString(16)); offset += 2; }
        host = segments.join(':');
    } else { throw new Error(`Unsupported address type: ${addressType}`); }
